...
func (a *Agent) ExecuteTaskMultiStep(ctx context.Context, taskText string, maxSteps int) error {
	...
	if err != nil {
		a.log.Error("Ошибка получения начального snapshot", zap.Error(err))
	...
	if err != nil {
		a.log.Error("Ошибка планирования multi-step", zap.Error(err))
...
func (a *Agent) executeMultiStepPlanWithMemory(ctx context.Context, taskText string, plan *llm.MultiStepPlan, maxSteps int, domain string) error {
	...
	if err == nil && a.memory != nil {
		...
		if saveErr := a.memory.RecordSuccess(ctx, taskText, plan.Steps, plan.OverallStrategy, duration, domain); saveErr != nil {
			a.log.Warn("Не удалось сохранить успешный путь", zap.Error(saveErr))
		} else {
			a.log.Info("Успешный путь сохранен в память", zap.Duration("duration", duration))
		}
	}
...
func (a *Agent) executeMultiStepPlan(ctx context.Context, taskText string, plan *llm.MultiStepPlan, maxSteps int, domain string) error {
	for stepNo, step := range plan.Steps {
		...
		if step.Action == "complete" {
			a.log.Info("Задача завершена согласно плану")
			return nil
		}

		pageSnapshot, err := a.browser.GetPageSnapshot(ctx)
		if err != nil {
			a.log.Warn("Не удалось получить snapshot перед шагом", zap.Error(err))
		}

		isDangerous, llmMessage, err := a.securityChecker.IsDangerousAction(ctx, step.Action, step.Selector, step.Value, step.Reasoning)
		if err != nil {
			a.log.Warn("Ошибка проверки безопасности", zap.Error(err))
		} else if isDangerous {
			if a.userInputProvider == nil {
				a.log.Warn("Опасное действие обнаружено, но провайдер не настроен", zap.String("action", step.Action))
			} else {
				...
				if answerLower != "yes" && answerLower != "y" && answerLower != "да" && answerLower != "д" {
					a.log.Info("Пользователь отменил опасное действие", zap.String("action", step.Action))
		...
		if err != nil {
			a.log.Error("Ошибка выполнения шага", zap.Int("step", stepNumber), zap.String("action", step.Action), zap.Error(err))

			if isCriticalError(err) {
				a.log.Error("Критическая ошибка, требуется replan", zap.Error(err))
				...
				if replanErr != nil {
					a.log.Error("Не удалось создать новый план", zap.Error(replanErr))
					return fmt.Errorf("failed to replan after error: %w", replanErr)
				}

				a.log.Info("Новый план создан после ошибки", zap.Int("new_steps", len(newPlan.Steps)))
			...
			a.log.Warn("Некритическая ошибка, продолжаем выполнение", zap.Error(err))
			fmt.Printf("[Шаг %d] Ошибка: %v (продолжаем)\n", stepNumber, err)

			if a.memory != nil {
				recovery := a.memory.GetFailureRecovery(ctx, step.Action, step.Selector, err.Error())
				if recovery != "" {
					a.log.Info("Найдена стратегия восстановления в памяти", zap.String("recovery", recovery))
